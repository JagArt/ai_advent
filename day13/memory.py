...
Состояние задачи — тоже не уровень памяти, а task.py. Память отвечает на вопрос
«о чём договорились», состояние — «где идёт работа». Связь между ними всё же есть,
и односторонняя: условия шагов автомата считаются по разделам рабочей памяти, а
память о состоянии не знает ничего.
"""
