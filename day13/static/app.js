...
const SESSION_KEY = "day13.session";
const WINDOW_KEY = "day13.window";
const AUTOSAVE_KEY = "day13.autosave";
...
// он взялся, нельзя.
function renderCards(proposals, moves) {
    const follow = isNearBottom(chatEl);
    cards = new Map();
    ...
    const build = (item) => (item.name ? moveCard(item) : proposalCard(item));
    for (const [answerId, items] of byAnswer(proposals, moves)) {
...
function byAnswer(proposals, moves) {
    const groups = new Map();
    for (const item of [...proposals, ...(moves ?? [])]) {
        const key = item.answer_id || 0;
        groups.set(key, [...(groups.get(key) ?? []), item]);
    }
    // Переход после находок внутри своей группы: id у двух очередей свои, и порядок
    // склейки сам по себе ничего не гарантирует.
    for (const items of groups.values()) {
        items.sort((one, two) => Number(Boolean(one.name)) - Number(Boolean(two.name)));
    }
    return groups;
}
...
async function forgetItem(target, itemId) {
    ...
    try {
        ...
        renderCards(payload.proposals, payload.moves);
    ...
    while (true) {
        ...
        while ((boundary = buffer.indexOf("\n\n")) !== -1) {
            ...
            if (event === "memory") {
                ...
                setMemory(update);
                renderCards(update.proposals, update.moves);
...
function showState(state) {
    ...
    // предложения по нему.
    renderCards(state.proposals, state.moves);
