...
const SESSION_KEY = "day14.session";
const WINDOW_KEY = "day14.window";
const AUTOSAVE_KEY = "day14.autosave";
...
// Состояние задачи целиком, вместе с шагами и допустимыми переходами: считает их
// сервер той же функцией, которой потом проверяет нажатие.
let task = null;
...
// какой инвариант сработал. Пузырь тот же — ответ на этот вопрос всё ещё один, просто
// написан он будет со второй попытки.
function breakAnswer(bubble, payload) {
    const note = document.createElement("div");
    note.className = "answer-broken";
    note.textContent = payload.retry
        ? `оборвано на инварианте ${brokenLabels(payload)} — ответ переписывается`
        : `оборвано на инварианте ${brokenLabels(payload)} — дальше отказ, дописанный кодом`;
...
// он взялся, нельзя.
function renderCards(proposals, moves, violations) {
    const follow = isNearBottom(chatEl);
    cards = new Map();
    ...
    const build = (item) => (item.label ? violationCard(item) : item.name ? moveCard(item) : proposalCard(item));
    for (const [answerId, items] of byAnswer(proposals, moves, violations)) {
...
function byAnswer(proposals, moves, violations) {
    const groups = new Map();
    for (const item of [...proposals, ...(moves ?? []), ...(violations ?? [])]) {
        const key = item.answer_id || 0;
        groups.set(key, [...(groups.get(key) ?? []), item]);
    }
    // Порядок внутри группы: нарушение, находки, переход. Нарушение первым потому, что
    // оно про сам ответ, под которым стоит, а остальные — про то, что из него следует.
    // Id у трёх очередей свои, и порядок склейки сам по себе ничего не гарантирует.
    const weight = (item) => (item.label ? 0 : item.name ? 2 : 1);
...
async function forgetItem(target, itemId) {
    ...
    try {
        ...
        renderCards(payload.proposals, payload.moves, payload.violations);
    ...
    while (true) {
        ...
        while ((boundary = buffer.indexOf("\n\n")) !== -1) {
            ...
            if (event === "memory") {
                ...
                setMemory(update);
                renderCards(update.proposals, update.moves, update.violations);
...
function showState(state) {
    ...
    // предложения по нему.
    renderCards(state.proposals, state.moves, state.violations);
