...
class Agent:
        ...
        async with self._lock:
            ...
            try:
                ...
                for number in range(RETRIES + 1):
                    ...
                    # код: короткий отказ с именем инварианта и с альтернативой из него.
                    # Нарушающий ответ пользователь не увидит ни в этом случае, ни в
                    # каком другом: в худшем случае он увидит отказ без модели.
                    last = number >= RETRIES
                    hits = attempt.hits
    ...
    async def _stream(
        ...
        """Ответ потоком, с детектором на ходу.
        ...
        — отправили». Пока в недописанном предложении нет запрещённого слова, оно
        уходит пользователю сразу, кусками, как в любом чате. Появилось — предложение
        задерживается до точки, потому что до точки «на Flask мы переходить» и «на
        Flask мы переходить не будем» — один и тот же текст. Дописалось — решает
        детектор: нарушение обрывает проход молча, отказ уходит на экран целиком.
        """
        parts: list[str] = []
        ...
        async for delta in stream_chat(
            ...
            ready = self._ready(rules, attempt.text)
    ...
    @staticmethod
    def _ready(rules: tuple[Rule, ...], text: str) -> int:
        """Сколько символов ответа можно отдать пользователю прямо сейчас."""
        if not rules:
            return len(text)

        settled = len(invariants.settled(text))
        return settled if invariants.mentions(rules, text[settled:]) else len(text)
    ...
    async def _propose(
        ...
        """Разбор хода: что записать, не вышел ли ответ за рамки и куда двинуть автомат.

        Три запроса, а не один: «куда положить сказанное», «нарушены ли инварианты» и
        «сделано ли ожидаемое действие» — разные вопросы, и в одном JSON они путаются.
        Модель начинает двигать этап потому, что нашла что записать, и не замечать
        нарушения потому, что решила двинуть этап.

        Раундов при этом два, а не три. Находки и аудит уходят одновременно: они друг о
        друге ничего не знают. Переход ждёт находок, и порядок здесь важнее скорости —
        условие шага считается по ТЗ, а ТЗ пополняется как раз находками этого хода:
        спроси про переход одновременно, и модель увидит «закрыто» там, где условие уже
        выполнено, и каждый шаг будет отставать на ход. Пользователь в это время читает
        ответ, поэтому второй раунд ему не виден.
        ...
        # Разбор находок и аудит уходят одновременно: аудит не зависит ни от находок,
        # ни от ТЗ — он про уже отданный ответ, — поэтому ход от него не длиннее.
