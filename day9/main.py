...
import memory as memory_policy
import tokens
from agent import (
    BUDGET_OPTIONS,
    COMPRESSION,
...
async def defaults() -> dict[str, Any]:
    return {
        ...
        "tail_messages": memory_policy.TAIL_MESSAGES,
        "compress_every": memory_policy.COMPRESS_EVERY,
        "summary_max_tokens": memory_policy.SUMMARY_MAX_TOKENS,
...
async def session(
    ...
    compression: bool = Query(default=COMPRESSION),
) -> dict[str, Any]:
    agent = await registry.get(session_id)
    # Бюджет и сжатие приходят со страницы: что попадёт в окно — конспект с хвостом
    # или вся история под бюджет, — зависит от них, а не от настроек по умолчанию.
    plan = await agent.plan(budget=context_budget, compression=compression)
    return {
...
def memory(plan: AgentPlan) -> dict[str, Any]:
    """Состояние памяти: вся история, окно контекста и конспект вместо остального."""
    return {
        ...
        "summary_size": plan.summary_messages,
        "summary_tokens": plan.summary_tokens,
        "pending_size": plan.pending_messages,
    }
...
async def event_stream(
    ...
    try:
        async for event in events:
            if isinstance(event, AgentPlan):
                ...
                # Свёртка случилась уже после ответа: страница узнаёт о ней отдельным
                # кадром, чтобы было видно, за что списаны лишние токены.
                yield sse_frame(
                    {
                        "messages": event.messages,
                        "summary_size": event.summary_messages,
    ...
    # Заголовок диалога и свёртка — тоже запросы к модели, поэтому таблица ходов
    # и итоги берутся из базы: там уже лежат все строки этого хода.
    turns = await agent.turns()
    yield sse_frame(
        {
            ...
            "turns": turns,
            **memory(await agent.plan(budget=budget, compression=compression)),
        },
        event="done",
    )
...
async def ask(request: AskRequest) -> StreamingResponse:
    ...
    return StreamingResponse(
        event_stream(
            agent,
            agent.ask(request.prompt.strip(), request.context_budget, request.compression),
