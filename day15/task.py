...
Слоем выше лежит gates.py: между этапами стоят гейты, и последнее слово о переходе
вперёд — за ними. Поэтому moves() здесь остаётся ответом автомата, а ответом
приложения становится gates.moves() — то же самое, но с закрытыми гейтами.
"""
...
class Task:
    ...
    def as_dict(self, counts: dict[str, int]) -> dict[str, object]:
        ...
        return {
            ...
            # Переходов здесь нет намеренно, хотя в day13 они были. Между этапами
            # теперь стоят гейты, и на вопрос «куда можно» есть ровно один ответ —
            # gates.moves(). Оставить рядом ответ автомата значило бы отдать странице
            # список кнопок, который не совпадает с проверкой на сервере.
        }
