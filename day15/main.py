...
class ApprovalRequest(BaseModel):
    """Решение по гейту подтверждения: утвердить или снять утверждение.

    Заметка необязательна и в проверке не участвует: «подтвердить с замечанием» — это
    всё-таки подтверждение, и превращать замечание в условие значило бы придумать
    третий тип гейта, которого в этом приложении нет.
    """
...
async def defaults() -> dict[str, Any]:
    return {
        ...
        # Гейты и области этапов: описание процесса, а не состояния задачи. Страница
        # рисует по ним подсказки и таблицу гейтов, а «пройден ли он» приходит снимком
        # сессии — по той же причине, по которой сюда не попадает текущий этап.
        "gates": gates.defaults(),
        "gate_kinds": [{"key": kind, "name": name} for kind, name in KIND_NAMES.items()],
        "scopes_by_stage": ahead.defaults(),
...
@app.post("/api/session/{session_id}/gates/{gate}")
async def approve_gate(
    ...
    """Утвердить гейт или снять утверждение. Единственная ручка, куда агент не ходит.

    POST, а не PUT: утверждение — событие, а не поле состояния. Их у гейта может быть
    несколько за жизнь задачи — план утвердили, откатились, утвердили снова, — и каждое
    остаётся в журнале отдельной строкой.

    Второго способа пройти гейт подтверждения нет: ни `PUT .../task`, ни карточка
    перехода, ни автосохранение мимо этой ручки не проходят.
    """
    agent = await registry.get(session_id)
    try:
        state = await agent.approve(gate, request.approved, request.note)
    except LookupError as error:
        raise HTTPException(status_code=404, detail=str(error)) from error
    except ValueError as error:
        raise HTTPException(status_code=422, detail=str(error)) from error
...
@app.get("/api/session/{session_id}/gates")
async def approvals(session_id: str) -> dict[str, Any]:
    """Журнал утверждений: кто и когда открыл гейт, и что унёс потом откат."""
    if not await storage.session_exists(session_id):
        raise HTTPException(status_code=404, detail="Сессия не найдена")
    return {"approvals": await storage.load_approvals(session_id)}
...
@app.get("/api/session/{session_id}/overruns")
async def overruns(session_id: str) -> dict[str, Any]:
    """Журнал забегов вперёд: где агент взялся за работу закрытого этапа и чем поймали."""
    if not await storage.session_exists(session_id):
        raise HTTPException(status_code=404, detail="Сессия не найдена")
    return {"overruns": await storage.load_overruns(session_id)}
...
def context(plan: AgentPlan) -> dict[str, Any]:
    ...
    return {
        ...
        # Гейты — дробью: сколько открыто из тех, что стоят прямо перед задачей. Рядом
        # число закрытых видов работы: по нему видно, есть ли сейчас куда забегать.
        "gates_open": plan.gates_open,
        "gates_total": plan.gates_total,
        "closed_work": plan.closed_work,
    }
                ...
                yield sse_frame(
                    {
                        "shown": event.shown,
                        "retry": event.retry,
                        # Чем оборван проход: рамкой или этапом. Страница пишет на месте
                        # снятого текста разное — «оборвано на инварианте #2» и
                        # «оборвано: работа этапа исполнение», — и путать их нельзя:
                        # первое снимает пользователь, второе открывает гейт.
                        "kind": event.kind,
                        "violations": [violation.as_dict() for violation in event.violations],
                        "overruns": [overrun.as_dict() for overrun in event.overruns],
                    },
                    event="broken",
                )
                ...
                yield sse_frame(
                    {
                        ...
                        # Просьба утвердить гейт: она появляется там, где переход
                        # держит только решение пользователя, и прячется в
                        # «отброшенном предложении» была бы ровно тем, чего от него
                        # ждут, — незамеченным.
                        "asked": event.gate.id if event.gate else 0,
                        "gate": event.gate.gate if event.gate else "",
                        # Нарушения и забеги, которые нашли аудиторы в уже отданном
                        # ответе, и находки, которые не стали карточками.
                        "caught": [violation.id for violation in event.violations],
                        "jumped": [overrun.id for overrun in event.jumps],
