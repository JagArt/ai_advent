...
# Четвёртый ответ на вопрос «куда сохранить» — «никуда»: отказ такое же решение,
# как и выбор уровня, и обрабатывается там же.
SKIP = "skip"
TARGETS = (*TIERS, SKIP)
...
SECTIONS = {
    LONGTERM: ("профиль", "решения", "знания"),
...
# о пользователе и о задаче. Каждый уровень — своё сообщение: разделение памяти
# видно и в том, что уходит в запрос.
INTROS = {
    LONGTERM: (
        "Долговременная память — то, что верно независимо от текущей задачи.\n"
        "Профиль пользователя, его решения и знания о его окружении:\n"
    ),
    WORKING: (
