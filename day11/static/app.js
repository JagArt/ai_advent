...
const SESSION_KEY = "day11.session";
const WINDOW_KEY = "day11.window";
const AUTOSAVE_KEY = "day11.autosave";

const LONGTERM = "longterm";
const WORKING = "working";

const TIER_NAMES = { [LONGTERM]: "долговременная", [WORKING]: "рабочая" };
const TIER_BUTTONS = { [LONGTERM]: "В долговременную", [WORKING]: "В рабочую" };

// Параметры агента, разделы памяти и шкала окна приходят из GET /api/defaults.
let config = null;
let sessionId = null;
let historySize = 0;
let dialogs = [];
// Два уровня, которые живут дольше реплики: их и рисуют блоки над лентой.
let memory = { [LONGTERM]: [], [WORKING]: [] };
...
function setBusy(busy) {
    ...
    // Память во время хода не трогаем: блоки уже ушли в запрос, и правка на этом
    // месте разошлась бы с тем, что видит модель.
...
function setMemoryDisabled(disabled) {
    for (const control of document.querySelectorAll(".tier-item-delete")) {
...
// в запрос и сколько пунктов лежит в блоках памяти рядом с ними.
...
function renderTier(tier, bodyEl, metaEl) {
    ...
    if (!items.length) {
        ...
        empty.textContent = tier === LONGTERM
            ? "Пусто — агент начинает разговор, ничего не зная о собеседнике"
...
// Оба блока перерисовываются вместе: перенос пункта меняет сразу два уровня, и
// рисовать их по отдельности значило бы показать память в промежуточном виде.
function setMemory(payload) {
    memory = { [LONGTERM]: payload.longterm, [WORKING]: payload.working };
...
// Пункт памяти узнаётся по паре «уровень и id»: один и тот же номер строки может
// встретиться на обоих уровнях, таблицы-то разные.
...
function stored() {
    return new Set(markers({ longterm: memory[LONGTERM], working: memory[WORKING] }));
}
...
function proposalCard(proposal) {
    ...
    const card = {
        ...
        // правку записывает кнопка уровня, а не сам ввод.
    ...
    for (const tier of [LONGTERM, WORKING]) {
        const button = document.createElement("button");
        button.type = "button";
        button.className = "button proposal-button";
        button.textContent = TIER_BUTTONS[tier];
        button.addEventListener("click", () => decide(card, tier));
        card.buttons[tier] = button;
...
// рекомендация агента, после — тот уровень, куда пункт лёг, и карточка сжимается
// в строку. Разворачивают её кнопкой, когда решение хотят поменять.
function paintCard(card) {
    const tier = card.saved || card.tier;
    ...
    card.state.textContent = card.saved
        ? `Сохранено в ${TIER_NAMES[card.saved]}`
        : "Сохранить в память?";
    ...
    card.badge.textContent = card.saved ? `${TIER_NAMES[card.saved]} · ${card.section}` : "";
    ...
    for (const [name, button] of Object.entries(card.buttons)) {
        ...
        // Уже сохранённый уровень не предлагается повторно: на его кнопке теперь
        // написано, что пункт там, а нажимают вторую — чтобы перенести.
        button.textContent = card.saved === name ? "Здесь" : TIER_BUTTONS[name];
...
async function decide(card, target) {
    ...
    try {
        ...
        if (!updated) {
            ...
            setStatus("Отклонено, в память не записано");
            return;
        }
        ...
        card.expanded = false;
        paintCard(card);
        setStatus(`Записано в ${TIER_NAMES[updated.saved]} · ${updated.section}`
...
async function forgetItem(tier, itemId) {
    if (locked()) {
        return;
    }

    try {
        const payload = await send(`/api/session/${sessionId}/memory/${tier}/${itemId}`, null, "DELETE");
        applyMemory(payload);
        ...
        setStatus(`Забыто из ${TIER_NAMES[tier]}`);
    } catch (error) {
        setStatus(error.message, true);
    }
}
...
function promptStatus(payload) {
    const parts = [`В запросе ${payload.context_size} ${messageWord(payload.context_size)}`];
    const stored = payload.longterm_size + payload.working_size;
    if (stored > 0) {
        parts.push(`плюс память ${stored} ${itemWord(stored)}`);
    }
...
// память с карточками заполняется на экране обычным путём.
async function startAutorun() {
    ...
    setBusy(false);
    setStatus("готовим чистый диалог, уровень для находок выбирает агент");
    ...
    try {
        ...
        // Уровень на время прогона выбирает агент: к кнопке в карточке прогон не
...
newDialogButton.addEventListener("click", async () => {
    ...
    try {
        await createSession();
        setStatus("Новый диалог: рабочая память пуста, долговременная на месте");
    } catch (error) {
        setStatus(error.message, true);
    }
    promptInput.focus();
...
resetButton.addEventListener("click", async () => {
    ...
    if (!confirm("Сбросить базу до исходной? Диалоги и всё, что добавили в память, пропадут.")) {
        return;
    }
    ...
    try {
        ...
        setStatus(`База сброшена: ${seeded} ${itemWord(seeded)} seed, диалоги пустые`);
    } catch (error) {
        setStatus(error.message, true);
    }
    resetButton.disabled = locked();
...
function settings() {
    return new URLSearchParams({ window_messages: windowMessages });
}
...
async function send(path, body = null, method = "POST") {
...
async function init() {
    ...
    autosaveInput.addEventListener("change", () => {
        ...
        setStatus(autosave
            ? "Уровень выбирает агент — карточка покажет, куда он положил"
            : "Уровень выбираете вы: агент только предлагает");
    });
    ...
    autorunButton.title = `Прогон ${config.autorun.length} реплик в новом диалоге:`
        + " уровень для находок на это время выбирает агент";
    setAutorunButton();
}
